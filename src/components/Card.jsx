...
function Card({ id }) {
  ...
  function turn() {
    setCardData((prevCardData) => {
      return [...prevCardData.slice(0, id), { ...prevCardData[id], isFlipped: true }, ...prevCardData.slice(id + 1)];
    });
  }
  ...
  return (
    <CardDiv
      className={`card ${cardData[id].isFlipped ? "turnAround" : "hidden"}`}
      style={cardData[id].isFlipped ? turnedStyle : null}
      onClick={
        cardData[id].isFound
          ? () => {
              return false;
